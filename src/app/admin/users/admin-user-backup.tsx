...
export default function AdminUsersClient() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredUsers.map((user) =>
      Object.keys(visibleColumns)
        .filter((key) => visibleColumns[key as keyof typeof visibleColumns])
        .map((key) => {
          if (key === "registeredAt" || key === "lastLoginAt") {
  ...
  // Virtualized Table Columns with Robust Date Handling
      ...
      ? [
          {
            ...
            valueFormatter: (params) => {
              ...
              console.log("RegisteredAt value:", value);
              if (!value) return "Never";
              const date = new Date(value);
              if (isNaN(date.getTime())) {
                console.log("Invalid date for registeredAt:", value);
                return "Invalid Date";
              }
      ...
      ? [
          {
            ...
            valueFormatter: (params) => {
              const value = params?.value as string;
              console.log("LastLoginAt value:", value);
              if (!value) return "Never";
              const date = new Date(value);
              if (isNaN(date.getTime())) {
                console.log("Invalid date for lastLoginAt:", value);
                return "Invalid Date";
              }
              return date.toLocaleString();
            },
                ...
                <Box sx={{ height: 400, width: "100%", overflow: "auto" }}>
                  <DataGrid
                    ...
                    onSelectionModelChange={(newSelection) => {
                      const selectedIds = Array.isArray(newSelection) ? newSelection.map(Number) : [];
