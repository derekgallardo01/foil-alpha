        ...
        await prisma.$transaction(async (tx) => {
          ...
          for (const losingBid of losingBids) {
            const loserWallet = await tx.userWallet.findUnique({
              where: { user_id: losingBid.bidderId }, // Fixed: bidder_id → bidderId
            });
            ...
            if (loserWallet) {
              ...
              await tx.walletTransaction.create({
                data: {
                  user_id: losingBid.bidderId, // Fixed: bidder_id → bidderId
    ...
    return NextResponse.json({
      ...
      auctions: endedAuctions.map((auction) => ({
        ...
        days_since_end: Math.floor(
          (now.getTime() - auction.auction_end!.getTime()) / (1000 * 60 * 60 * 24),
        ),
      })),
    });
  } catch (error) {
