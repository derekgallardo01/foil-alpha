...
export async function POST(request: NextRequest) {
  try {
    ...
    if (currentHighestBid && bidAmount <= Number(currentHighestBid.amount)) {
      return NextResponse.json(
        {
          error: `Bid must be higher than current highest bid of $${Number(
    ...
    if (availableBalance < bidAmount) {
      return NextResponse.json(
        {
          error: `Insufficient available balance. Available: $${availableBalance.toFixed(
            2,
          )}, Required: $${bidAmount.toFixed(2)}`,
        },
        { status: 400 },
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (previousBidFromUser) {
        ...
        await tx.walletTransaction.create({
          data: {
            user_id: bidderId,
            wallet_id: bidderWallet.id, // Added
      ...
      if (currentHighestBid && currentHighestBid.bidderId !== bidderId) {
        const previousBidderWallet = await tx.userWallet.findUnique({
          where: { user_id: currentHighestBid.bidderId },
        });
        ...
        if (previousBidderWallet) {
          ...
          await tx.walletTransaction.create({
            data: {
              user_id: currentHighestBid.bidderId,
              wallet_id: previousBidderWallet.id, // Added
              transaction_type: 'UNFREEZE_FUNDS',
              amount: previousAmount,
