// import NextAuth, { AuthOptions } from 'next-auth'; // Add AuthOptions import
...
// src/app/api/auth/[...nextauth]/route.ts - COMPLETE file with dev bypass
...
export const authOptions: AuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // DEV MODE: Check dev users first
        if (isDevMode()) {
          const devUser = getDevUser(email);
          if (devUser && password === devUser.password) {
            console.log('🚧 DEV AUTH: Logging in dev user:', email);
            return {
        ...
        // PRODUCTION: Normal database authentication
        const pool = await getDbConnection();
        try {
          const [rows]: [RowDataPacket[], unknown] = await pool.execute(
            'SELECT id, name, email, password, role, subscriptionStatus, is_verified FROM users WHERE email = ?',
          ...
          console.error('Authorize error:', error);
          if (isDevMode()) {
            // In dev mode, be more permissive with errors
            console.warn('🚧 DEV MODE: Auth error ignored:', error);
            return null;
          } else {
            throw error;
          }
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      // Skip complex OAuth flow in dev mode for credentials provider
      if (isDevMode() && account?.provider === 'credentials') {
        return true;
      }
      ...
      try {
        ...
        if (account?.provider === 'discord') {
          ...
          if (rows.length === 0) {
            ...
            if (!isDevMode() && !dbUser.is_verified) {
              if (!dbUser.verification_code) {
                await pool.execute(
                  'UPDATE users SET verification_code = ? WHERE email = ?',
                  [verificationCode, user.email]
        ...
        if (userRows.length > 0 && !isDevMode() && !userRows[0].is_verified) {
          return `/verify-email?email=${encodeURIComponent(user.email!)}`;
        }
