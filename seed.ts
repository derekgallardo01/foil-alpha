...
async function main() {
  ...
  const charley = await prisma.user.findUnique({ where: { email: 'charley@test.com' } });

  if (!admin || !bob || !john || !charley) {
  ...
  await prisma.userWallet.createMany({
    data: [
      { user_id: admin.id, balance: 10000.00, frozen_balance: 0.00 },
      { user_id: bob.id, balance: 500.00, frozen_balance: 0.00 },
      { user_id: john.id, balance: 750.00, frozen_balance: 0.00 },
      { user_id: charley.id, balance: 300.00, frozen_balance: 0.00 },
    ],
    skipDuplicates: true,
  });
  ...
  // Fetch references for foreign keys
  ...
  const stage2Subtype = await prisma.subtype.findUnique({ where: { name: 'Stage 2' } });
  ...
  if (!baseSet || !commonRarity || !holoRarity || !pokemonSupertype || !basicSubtype || !stage2Subtype || !fireType || !waterType || !electricType) {
    throw new Error('Failed to find required foreign key references for cards');
  }
  ...
  const sampleCards = [
    ...
    },
    {
      ...
      primary_type_id: electricType.id,
      market_price: 25.00,
      source: CardSource.MANUAL,
    },
  ];
  ...
  console.log('   🎯 Supertypes: 3 card supertypes');
  console.log('   🏷️ Subtypes: 23 card subtypes');
